/*
Pulled straight from Samples/Common/ExampleLoadingBar.h
...
#ifndef __PLANET_LOADING_BAR__
#define __PLANET_LOADING_BAR__
...
*/
class PlanetLoadingBar : public ResourceGroupListener
{
protected:
...
public:
		...
		Real initProportion = 0.20f)
	{
		...
		if (!mLoadOverlay)
		{
			OGRE_EXCEPT(Exception::ERR_ITEM_NOT_FOUND, 
				"Cannot find loading overlay", "PlanetLoadingBar::start");
		}
		mLoadOverlay->show();
	...
	*/
	...
	void resourceGroupScriptingStarted(const String& groupName, size_t scriptCount)
	{
		...
		mLoadingDescriptionElement->setCaption(groupName + " started...");
		mWindow->update();
	};
	...
	{
		mLoadingCommentElement->setCaption(scriptName + " parsing...");
		mWindow->update();
	};
	...
	void scriptParseEnded(const String& scriptName)
	{
		...
		mLoadingDescriptionElement->setCaption(scriptName + " finished...");
		mWindow->update();
	};
	...
	{
		mLoadingDescriptionElement->setCaption(groupName + " complete...");
	};
	...
	void resourceGroupLoadStarted(const String& groupName, size_t resourceCount)
	{
		...
		mLoadingDescriptionElement->setCaption(groupName + " loading...");
		mWindow->update();
	};
	...
	{
		mLoadingDescriptionElement->setCaption("Processing resources...");
